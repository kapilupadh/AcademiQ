// client/src/components/layout/TeacherSidebar.jsx
import React, { useState, useEffect } from "react";
import { Link, useLocation, useNavigate } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import {
  LayoutDashboard, BookOpen, Users, NotebookPen, Clock,
  CalendarRange, FileText, BookMarked, MessageSquare,
  ChevronLeft, ChevronRight, LogOut, School, Menu, X,
} from "lucide-react";
...
const SECONDARY = [
  // ✅ Attendance is now LIVE — comingSoon removed
...
const T = {
  dark: {
    ...
    navActive: "bg-teal-500/12 text-teal-400", activeBar: "bg-teal-500",
  ...
  light: {
    ...
    navActive: "bg-teal-50 text-teal-700", activeBar: "bg-teal-600",
...
function NavItem({ icon: Icon, label, to, badge, comingSoon, collapsed, tk, onClick }) {
  const { pathname } = useLocation();
  const isActive = !comingSoon && (pathname === to || (to !== "/teacher/dashboard" && to.length > 1 && pathname.startsWith(to)));

  if (comingSoon) {
    return (
      <div className="relative group">
        <div className={`flex items-center gap-3 rounded-lg text-sm font-medium cursor-not-allowed opacity-50 select-none
          ${collapsed ? "justify-center px-0 py-2.5 w-full" : "px-3 py-2.5"} ${tk.navBase}`}
          style={{ fontFamily: "'DM Sans', sans-serif" }}>
          <span className="relative shrink-0">
            <Icon size={17} strokeWidth={1.8} />
            {collapsed && badge && <Badge badge={badge} forIcon />}
          </span>
          {!collapsed && (
            <>
...
function SidebarContent({ collapsed, onToggleCollapsed, onItemClick, showClose, onClose, mobileView }) {
  ...
  const handleLogout = () => { localStorage.removeItem("token"); localStorage.removeItem("user"); navigate("/teacher-login"); };
    ...
    <div className={`flex flex-col h-full transition-colors duration-150 ${tk.bg} ${tk.border} border-r`} style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className={`flex items-center justify-between border-b ${tk.divider} h-16 shrink-0 ${collapsed ? "px-2" : "px-3"}`}>
        {!collapsed ? (
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            ...
            <div className="w-8 h-8 rounded-md bg-teal-600 items-center justify-center shrink-0 hidden"><School size={15} className="text-white" /></div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <p className={`text-sm font-bold leading-tight ${tk.brand}`}>AcademiQ</p>
                <span className={`text-[9px] font-semibold px-1.5 py-0.5 rounded-full leading-none ${tk.rolePill}`}>TEACHER</span>
              </div>
              <p className={`text-[10px] leading-tight ${tk.brandSub}`}>Faculty Portal</p>
            </div>
          </div>
        ) : (
          <>
      ...
      <div className={`flex-1 overflow-y-auto scrollbar-hide ${collapsed ? "px-2" : "px-3"} pb-2`}>
        ...
        <div className="space-y-0.5">
          {SECONDARY.map(item => <NavItem key={item.key} {...item} collapsed={collapsed} tk={tk} onClick={onItemClick} />)}
        </div>
      </div>
...
export default function TeacherSidebar() {
  const [collapsed, setCollapsed] = useState(() => { try { return JSON.parse(localStorage.getItem("tch_sidebar_collapsed")) ?? false; } catch { return false; } });
        ...
        {mobileOpen && (
          <>
            ...
            <motion.aside key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
              ...
              <SidebarContent collapsed={false} onToggleCollapsed={toggleCollapsed} onItemClick={() => setMobileOpen(false)} showClose onClose={() => setMobileOpen(false)} mobileView />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
      <motion.aside initial={false} animate={{ width: sidebarW }} transition={{ type: "spring", stiffness: 380, damping: 38 }} className="fixed left-0 top-0 bottom-0 z-40 hidden md:block overflow-hidden">
        <SidebarContent collapsed={collapsed} onToggleCollapsed={toggleCollapsed} onItemClick={null} showClose={false} onClose={null} mobileView={false} />
      </motion.aside>
    </>
  );
}