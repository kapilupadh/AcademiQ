...
const getSecondaryNav = (notificationsCount = 0) => [
  ...
  },
  {
    ...
    sub: [
      ...
      { label: "Department Locations", icon: MapPin, to: "/admin/departments/locations" }, // <--- 2. ADDED DEPARTMENT LOCATIONS HERE
  ...
  },
  {
    ...
    icon: MessageSquare,
    badge: { type: "count", text: String(notificationsCount || 0), style: "bg-violet-600 text-white" },
    sub: [
...
const T = {
  dark: {
    bg: "bg-[#0f1117]", border: "border-zinc-800", label: "text-zinc-600",
    divider: "border-zinc-800", navBase: "text-zinc-400",
    navHover: "hover:bg-zinc-800/60 hover:text-zinc-100",
    navActive: "bg-violet-500/12 text-violet-400", activeBar: "bg-violet-500",
    btn: "text-zinc-400 hover:text-zinc-100 hover:bg-zinc-800",
    brand: "text-white", brandSub: "text-zinc-500",
    ...
    userName: "text-zinc-100", email: "text-zinc-500",
  ...
  light: {
    bg: "bg-zinc-50", border: "border-zinc-200", label: "text-zinc-400",
    divider: "border-zinc-200", navBase: "text-zinc-600",
    navHover: "hover:bg-zinc-100 hover:text-zinc-900",
    navActive: "bg-violet-50 text-violet-700", activeBar: "bg-violet-600",
    btn: "text-zinc-500 hover:text-zinc-800 hover:bg-zinc-100",
    brand: "text-zinc-900", brandSub: "text-zinc-500",
    rolePill: "bg-violet-50 text-violet-700 border border-violet-200",
    avatar: "bg-violet-100 text-violet-700 border-violet-200",
    userName: "text-zinc-900", email: "text-zinc-500",
    tooltip: "bg-zinc-900 border-zinc-700 text-white shadow-xl",
...
function Badge({ badge, forIcon = false }) {
  ...
  if (forIcon && badge.type === "count") {
    return <span className={`absolute -top-1 -right-1 flex items-center justify-center w-3.5 h-3.5 rounded-full text-[9px] font-bold leading-none ${badge.style}`}>{badge.text}</span>;
  }
  if (forIcon) return <span className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-violet-400" />;
  if (badge.type === "count") return <span className={`inline-flex items-center justify-center w-4 h-4 rounded-full text-[10px] font-bold leading-none shrink-0 ${badge.style}`}>{badge.text}</span>;
  return <span className={`inline-flex items-center px-1.5 py-0.5 rounded-full text-[8px] font-bold tracking-wide uppercase leading-none shrink-0 ${badge.style}`}>{badge.text}</span>;
}
...
function Tooltip({ label, tk }) {
  return (
    <div className="absolute left-full ml-3 top-1/2 -translate-y-1/2 z-50 pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity duration-150">
      <div className={`px-2.5 py-1.5 rounded-md text-xs whitespace-nowrap border ${tk.tooltip}`}>{label}</div>
    </div>
  );
}
  ...
  return (
    <div className="relative group">
      <Link to={to} onClick={onClick}
        ...
        {isActive && !collapsed && <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full ${tk.activeBar}`} />}
        <span className="relative shrink-0">
          <Icon size={17} strokeWidth={isActive ? 2.2 : 1.8} />
          {collapsed && <Badge badge={badge} forIcon />}
        </span>
        {!collapsed && (<><span className="flex-1 truncate">{label}</span><Badge badge={badge} /></>)}
      </Link>
...
function SectionLabel({ text, collapsed, tk }) {
  ...
  return <p className={`text-[9px] font-bold uppercase tracking-[0.12em] px-3 mb-1.5 ${tk.label}`} style={{ fontFamily: "'DM Sans', sans-serif" }}>{text}</p>;
}

// Sub item — supports comingSoon flag
function SubItem({ icon: Icon, label, to, comingSoon, tk, onClick }) {
  ...
  if (comingSoon) {
    return (
      <div className={`flex items-center gap-2.5 px-2 py-1.5 rounded-md text-xs font-medium cursor-not-allowed opacity-50 select-none ${tk.subBase}`}
        style={{ fontFamily: "'DM Sans', sans-serif" }}>
        <Icon size={13} strokeWidth={1.8} className="shrink-0" />
        <span className="truncate flex-1">{label}</span>
        <span className={`text-[8px] font-bold tracking-wide uppercase px-1.5 py-0.5 rounded border leading-none shrink-0 ${tk.soon}`}>Soon</span>
      </div>
    );
  }
  ...
  return (
    <Link to={to} onClick={onClick}
      className={`flex items-center gap-2.5 px-2 py-1.5 rounded-md text-xs font-medium transition-all duration-150 ${isActive ? tk.subActive : tk.subBase}`}
      style={{ fontFamily: "'DM Sans', sans-serif" }}>
...
function AccordionGroup({ icon: Icon, label, groupKey, badge, openGroups, onToggle, tk, collapsed, subItems, onItemClick }) {
  ...
  const hasActiveSub = subItems.some(s => !s.comingSoon && (pathname === s.to || (s.to.length > 1 && pathname.startsWith(s.to))));
      ...
      {collapsed ? (
        <>
          <button className={`flex justify-center w-full py-2.5 rounded-lg transition-all duration-150 relative ${hasActiveSub ? tk.navActive : `${tk.navBase} ${tk.navHover}`}`}>
            <span className="relative"><Icon size={17} strokeWidth={hasActiveSub ? 2.2 : 1.8} />{badge && <Badge badge={badge} forIcon />}</span>
          </button>
        ...
        <>
          <button onClick={() => onToggle(groupKey)}
            className={`relative flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm font-medium transition-all duration-150 ${hasActiveSub ? tk.navActive : `${tk.navBase} ${tk.navHover}`}`}
            style={{ fontFamily: "'DM Sans', sans-serif" }}>
            {hasActiveSub && <span className={`absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-full ${tk.activeBar}`} />}
...
function SidebarContent({ collapsed, openGroups, onToggleGroup, onToggleCollapsed, onItemClick, showClose, onClose, mobileView, notificationsCount = 0 }) {
  ...
  const handleLogout = () => { localStorage.removeItem("token"); localStorage.removeItem("user"); navigate("/admin/login"); };
    ...
    <div className={`flex flex-col h-full transition-colors duration-150 ${tk.bg} ${tk.border} border-r`} style={{ fontFamily: "'DM Sans', sans-serif" }}>
      <div className={`flex items-center justify-between border-b ${tk.divider} h-16 shrink-0 ${collapsed ? "px-2" : "px-3"}`}>
        {!collapsed ? (
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            <img src={isDark ? LOGO_DARK_MODE : LOGO_LIGHT_MODE} alt="AcademiQ" className="h-8 w-auto rounded-md object-contain shrink-0"
              onError={e => { e.target.style.display = "none"; if (e.target.nextSibling) e.target.nextSibling.style.display = "flex"; }} />
            <div className="w-8 h-8 rounded-md bg-violet-600 items-center justify-center shrink-0 hidden"><School size={15} className="text-white" /></div>
          ...
          <>
            <img src={isDark ? LOGO_DARK_MODE : LOGO_LIGHT_MODE} alt="AcademiQ" className="w-8 h-8 rounded-md object-contain shrink-0"
              onError={e => { e.target.style.display = "none"; if (e.target.nextSibling) e.target.nextSibling.style.display = "flex"; }} />
            <div className="w-8 h-8 rounded-md bg-violet-600 items-center justify-center shrink-0 hidden"><School size={15} className="text-white" /></div>
          </>
        )}
        {!mobileView ? (
          <button onClick={onToggleCollapsed} className={`p-1.5 rounded-lg transition-all duration-150 ${tk.btn}`}>
            {collapsed ? <ChevronRight size={15} /> : <ChevronLeft size={15} />}
          </button>
        ) : showClose ? (
          <button onClick={onClose} className={`p-1.5 rounded-lg transition-all duration-150 ${tk.btn}`}><X size={15} /></button>
        ) : null}
      </div>
      ...
      <div className={`${collapsed ? "px-2" : "px-3"} pt-3 pb-1 overflow-hidden`}>
        ...
        <nav className="space-y-0.5">
          {PRIMARY.map(item => <NavItem key={item.key} {...item} collapsed={collapsed} tk={tk} onClick={onItemClick} />)}
        </nav>
      </div>
      ...
      <div className={`flex-1 overflow-y-auto scrollbar-hide ${collapsed ? "px-2" : "px-3"} pb-2`}>
        ...
        <div className="space-y-0.5">
          {getSecondaryNav(notificationsCount).map(item => (
            <AccordionGroup key={item.key} groupKey={item.key} icon={item.icon} label={item.label}
              badge={item.badge} subItems={item.sub} openGroups={openGroups} onToggle={onToggleGroup}
              tk={tk} collapsed={collapsed} onItemClick={onItemClick} />
          ))}
        </div>
      </div>
      ...
      <div className={`overflow-hidden ${collapsed ? "px-2 py-3 flex justify-center" : "px-3 py-3"}`}>
        {collapsed ? (
          <div className="relative group">
            <button onClick={handleLogout} title="Logout" className="p-1.5 rounded-md text-red-500 hover:bg-red-500/10 transition-all duration-150"><LogOut size={15} /></button>
          ...
          <>
            ...
            <div className="flex items-center gap-2.5">
              ...
              <button onClick={handleLogout} title="Logout" className="p-1.5 rounded-md text-red-500 hover:text-red-400 hover:bg-red-500/10 transition-all duration-150 shrink-0"><LogOut size={14} /></button>
            </div>
          </>
        )}
      </div>
...
export default function AdminSidebar() {
  const [collapsed, setCollapsed] = useState(() => { try { return JSON.parse(localStorage.getItem("adm_sidebar_collapsed")) ?? false; } catch { return false; } });
  ...
  return (
    <>
      <button onClick={() => setMobileOpen(true)} className="fixed top-4 left-4 z-50 md:hidden p-2 rounded-lg bg-zinc-900 border border-zinc-700 shadow-lg text-zinc-400 hover:text-white transition-all" aria-label="Open sidebar">
        <Menu size={18} />
      </button>
      <AnimatePresence>
        {mobileOpen && (
          <>
            ...
            <motion.aside key="drawer" initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }}
              transition={{ type: "spring", stiffness: 380, damping: 38 }} className="fixed left-0 top-0 bottom-0 z-50 w-[260px] shadow-2xl md:hidden">
              <SidebarContent collapsed={false} openGroups={openGroups} onToggleGroup={toggleGroup} onToggleCollapsed={toggleCollapsed}
                onItemClick={() => setMobileOpen(false)} showClose onClose={() => setMobileOpen(false)} mobileView notificationsCount={unreadCount} />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
      <motion.aside initial={false} animate={{ width: sidebarW }} transition={{ type: "spring", stiffness: 380, damping: 38 }}
        ...
        <SidebarContent collapsed={collapsed} openGroups={openGroups} onToggleGroup={toggleGroup} onToggleCollapsed={toggleCollapsed}
          onItemClick={null} showClose={false} onClose={null} mobileView={false} notificationsCount={unreadCount} />
      </motion.aside>
    </>
  );
}